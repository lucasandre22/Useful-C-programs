...
//It was made by git.com/lucasandre22
...
namespace List {
	...
	public:
		...
		~Iterator() { }
		void SetIteratorHead(List<T2>& list) { this->iterator = list.GetHead(); }
		T2 GetIterator() const { return this->iterator->GetValue(); }
		bool IteratorNull() const { return this->iterator == nullptr ? true : false; }
		void operator++() { this->iterator = this->iterator->next; }
		void operator=(List<T2>& list) { SetIteratorHead(list); }
	};
